...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
    ...
    var pizzaLength = pizzaSize.length;

    function changePizzaSizes(size) {
      //It defines the variable which is being updated from pizza slider
      var changedWidth;
      //Each case is checked for the slider and return the assigned changedWidth value
      switch(size) {
        case "1":
          changedWidth = 25;
          break;
      ...
      for (var i = 0; i < pizzaLength; i++) {
        pizzaSize[i].style.width = changedWidth + '%';
...
/*
  //Tried this method and resize timing was around 1~2 ms, also some FSL was happening

  function changePizzaSizes(size) {
    var i = 0;
    var pizzaSize = document.querySelectorAll(".randomPizzaContainer");
    var dx = determineDx(pizzaSize[i], size);
...
//rAF implementation found at Paul Lewis's great article:
...
function updatePositions() {
  ...
  //Length and yZ variables moved from for-loop
  //All the phases are pushed into an array
  var lengthH = items.length;
  var yZ = latestKnownScrollTop / 1250;
  var phaseArray = [];
  ...
  for (var i = 0; i < lengthH; i++) {
    ...
    // Changed items[i].style.left to transform and transalte to improve FPS from:https://discussions.udacity.com/t/optimizing-the-updatepositions-function/2834
    // Learned the effects from http://www.paulirish.com/2012/why-moving-elements-with-translate-is-better-than-posabs-topleft/
...
document.addEventListener('DOMContentLoaded',window.requestAnimationFrame(function() {
  ...
  var movingPizzas = document.querySelector("#movingPizzas1"); //Outside loop

  //This method didn't improve the performance much
  ...
  items = document.getElementsByClassName('mover');//Relocating the items variable
  window.requestAnimationFrame(updatePositions);
}));
